...
# Fixed hue order is the CVD-safety mechanism: it is never reordered per chart,
# and a chart never invents a 9th colour by cycling back to slot 1 on purpose
# (see PALETTE_MUTED note on _slot below for the one place that does cycle).
PALETTE = [
    "#2a78d6",  # 1 blue
    "#eb6834",  # 2 orange
    "#1baf7a",  # 3 aqua
...
.small-multiples { display: flex; flex-wrap: wrap; gap: 8px; }
...
def esc(text):
    ...
    Ticket summaries, display names and status labels come from a live Jira
    instance and routinely contain &, <, >, and quotes. Every primitive in
    this module runs every interpolated string through here before it lands
    in markup; skipping it for even one field turns a chart into a broken
    XML document, since it is data from a work Jira instance rather than
    text this module controls.
    """
...
def _is_num(v):
    # bool is a subclass of int; treat True/False as categorical, not a number,
    # so a boolean column doesn't silently become a numeric axis.
...
def _y_gridlines(values, left, right, top, bottom, plot_h, zero_floor):
    ...
    def sy(v, _lo=y_lo, _span=y_span):
        return bottom - (v - _lo) / _span * plot_h
...
def axes(rows, x, y, width, height, zero_floor=False):
    ...
    ys = [r.get(c) for r in rows for c in y_cols if _is_num(r.get(c))]

    y_markup, sy = _y_gridlines(ys, left, right, top, bottom, plot_h, zero_floor)

    numeric_x = bool(xs) and all(_is_num(v) for v in xs)
    parts = [y_markup]
    ...
    if numeric_x:
        ...
        def sx(v, _lo=x_lo, _span=x_span):
            return left + (v - _lo) / _span * plot_w
...
def _legend(names, y, x=46):
    """A left-aligned row of colour-key + label, one per series.

    dataviz: a legend is always present for two or more series (the
    dependable identity channel); a single series needs no legend box since
    the chart's own title already says what's plotted, so callers only
    invoke this when len(names) > 1.
    """
    parts = []
    cx = x
    for j, name in enumerate(names):
        ...
        # ponytail: advances the cursor by a rough characters-per-label estimate
        # instead of measured text width (no renderer available offline). Fine
        # for the short series names (status names, "done"/"open") this report
        # actually uses; a long series name would need real text metrics.
        cx += 18 + 7 * len(str(name)) + 16
    return "".join(parts)
...
def bars(rows, labels, series):
    ...
    width, height = 480, base_h + legend_h
    pad_l, pad_r, pad_t, pad_b = 46, 14, 12, 26
    plot_w = max(width - pad_l - pad_r, 1)
    plot_h = max(base_h - pad_t - pad_b, 1)
    left, bottom = pad_l, pad_t + plot_h

    values = [row.get(s) for row in rows for s in series if _is_num(row.get(s))]
    ...
    legend = _legend(series, base_h + 4) if len(series) > 1 else ""
...
def lines(rows, x, series):
    ...
    parts = [grid_markup]
    for j, s in enumerate(series):
        pts = [(sx(r.get(x)), sy(r.get(s))) for r in rows if _is_num(r.get(s))]
        if not pts:
            continue
        color = _slot(j)
        ...
        parts.append(
            ...
            f'stroke-linecap="round" stroke-linejoin="round"><title>{esc(s)}</title></path>'
        )
        lx, ly = pts[-1]
        parts.append(
...
def stacked(rows, x, band, value):
    ...
    to `value`. A band's colour comes from its first-seen order across the
    whole dataset, not its position within any one bar, so (dataviz
    anti-pattern) the same status always wears the same colour no matter
    which bar it appears in or what got filtered out.
    """
    if not rows:
        return '<p class="empty">no data</p>'
    ...
    totals = {}
    for r in rows:
        totals[r.get(x)] = totals.get(r.get(x), 0) + (r.get(value) or 0)
    v_max = max(totals.values()) if totals else 0

    base_h = 220
    legend_h = 22 if len(band_order) > 1 else 0
    width, height = 480, base_h + legend_h
    pad_l, pad_r, pad_t, pad_b = 46, 14, 12, 26
    plot_w = max(width - pad_l - pad_r, 1)
    plot_h = max(base_h - pad_t - pad_b, 1)
    ...
    for i, xv in enumerate(x_order):
        ...
        running = 0
        segs = {r.get(band): (r.get(value) or 0) for r in by_x[xv]}
        for b in band_order:
            v = segs.get(b, 0)
            if v <= 0:
                continue
            ...
            parts.append(
                ...
                f'height="{(y0 - y1):.1f}" fill="{color_of[b]}">'
                f"<title>{esc(b)}: {esc(v)}</title></rect>"
            )
            running += v
        parts.append(
    ...
    legend = _legend(band_order, base_h + 4) if len(band_order) > 1 else ""
...
def scatter(rows, x, y, guides=()):
    ...
    for r in rows:
        ...
        parts.append(
            ...
            f'stroke="var(--surface)" stroke-width="2">'
            f"<title>{esc(x)} {esc(vx)}, {esc(y)} {esc(vy)}</title></circle>"
        )
    ...
    for label, value in guides:
        ...
        parts.append(
            ...
            f'class="guide-line" stroke="{PALETTE_MUTED["baseline"]}" stroke-dasharray="4 3" />'
        )
        parts.append(
...
def table(rows, headers, shade=None):
    ...
    shade_max = 0
    if shade:
        shade_values = [r.get(shade) for r in rows if _is_num(r.get(shade))]
    ...
    for r in rows:
        ...
        for h in headers:
            ...
            if shade and h == shade and shade_max and _is_num(v):
                opacity = max(v, 0) / shade_max
                cells.append(
...
def small_multiples(groups, x, y):
    ...
    single y-scale so magnitude compares honestly across facets (the
    classic way small multiples mislead is giving each panel its own
    scale). `groups` is a mapping, or a sequence of (title, rows) pairs, of
    facet title to that facet's rows.
    """
    items = list(groups.items()) if hasattr(groups, "items") else list(groups)
    if not items:
        return '<p class="empty">no data</p>'

    values = [r.get(y) for _, facet_rows in items for r in facet_rows if _is_num(r.get(y))]
    ...
    for idx, (title, facet_rows) in enumerate(items):
        ...
        if not facet_rows:
            ...
            cats = list(dict.fromkeys(r.get(x) for r in facet_rows))
            n = len(cats)
            plot_l, plot_r = pad, fw - pad
            plot_t, plot_b = 18, fh - pad

            def fx(cat, _cats=cats, _n=n, _l=plot_l, _r=plot_r):
                if _n <= 1:
                    return _l + (_r - _l) / 2
                return _l + _cats.index(cat) / (_n - 1) * (_r - _l)

            def fy(v, _max=shared_max, _t=plot_t, _b=plot_b):
                return _b - (max(v, 0) / _max) * (_b - _t)

            pts = [(fx(r.get(x)), fy(r.get(y) or 0)) for r in facet_rows]
            d = " ".join(
