...
def sync(con, jira):
    ...
    for n, key in enumerate(wanted, start=1):
        ...
        con.execute(
            ...
            [key, issue["fields"]["updated"], datetime.now(timezone.utc), json.dumps(issue)],
        )
...
Note `_ts` must also accept the `...Z` form the Sprint field uses. Extend it:

```python
    if value.endswith("Z"):
        value = value[:-1] + "+00:00"
```
placed before the offset normalisation, and add a test:
