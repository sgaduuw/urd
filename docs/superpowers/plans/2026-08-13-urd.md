...
CHARTS = [
    ...
    Chart(
        ...
        options={"labels": "status", "series": None, "group": "type", "value": "days"},
        sql="""
...
def run_chart(con, chart):
    ...
    if chart.coverage:
        numerator, denominator = con.execute(chart.coverage).fetchone()
        share = 0 if not denominator else (numerator or 0) / denominator
        if share < chart.threshold:
            return render.coverage_strip(chart.title, numerator or 0, denominator or 0,
                                         chart.threshold)
