...
def sync(con, jira):
    ...
    for n, key in enumerate(wanted, start=1):
        try:
            ...
            con.execute(
                ...
                [key, datetime.now(timezone.utc), str(err)],
            )
        ...
        con.execute(
            ...
            [key, updated, datetime.now(timezone.utc), json.dumps(issue)],
        )
    ...
    save_scope(con, last_sync_at=datetime.now(timezone.utc).isoformat(timespec="seconds"))
...
def load_fixtures(con, *names):
    ...
    for name in names:
        ...
        con.execute(
            ...
            [issue["key"], issue["fields"]["updated"], datetime.now(timezone.utc), raw],
        )
