...
- **A mutation must be the literal pre-fix line**, recovered with `git checkout`, not
  a hand-authored break. Three rounds on Task 8 wrote mutations against the assumption
  the fix encoded rather than the state it replaced, and five tests that could not fail
  survived as a result, including one whose own docstring said it must prove generation
  "not merely document values the CSS template hard-codes on its own".
- **Anything that emits SVG asserts frame against content**: every mark and text
  coordinate within the declared `viewBox`, per primitive. Two defects shipped past a
  full suite, ruff and twenty-five mutations for want of this: an endpoint label drawn
  at x=472 in a 480-wide box, and a loop variable shadowing the chart height so a
  stacked chart's `viewBox` became its last segment. An accessibility test that checks
  a `viewBox` attribute *exists* does not check that it *contains* anything.
- **Structural verification proves numbers, not pixels.** Regex over geometry cannot
  see text advance width (every "the label fits" claim rests on a 7px-per-character
  estimate), occlusion and paint order (a rect with height 1 can be entirely painted
  over by the next one), overprint legibility, theme rendering, or sub-pixel loss.
  Frame-versus-content and occlusion are checkable in code and are required. The rest
  need a renderer, and one **is** available: the `claude-in-chrome` skill can open a
  local file and screenshot it. Use it from Task 9 onward, where there is a real HTML
  file to open, rather than deferring every visual class to Task 14.
...
```json
{
  ...
  "fields": {
    ...
    "customfield_20001": 5,
    "customfield_20002": [
      {"id": 7, "name": "Sprint B", "state": "closed",
...
```json
{
  ...
  "fields": {
    ...
    "customfield_20002": [
      {"id": 7, "name": "Sprint B", "state": "closed",
       "startDate": "2026-01-05T09:00:00.000Z", "endDate": "2026-01-19T09:00:00.000Z"},
      {"id": 3, "name": "Sprint A", "state": "closed",
...
def test_sprint_membership_is_ordered_so_carry_over_is_visible():
    ...
    # Array order is [id 7 "Sprint B", id 3 "Sprint A"], which disagrees with both
    # id order and name order, so this pins the ordinal to array position.
    assert rows == [("Sprint B", 1), ("Sprint A", 2)]
...
def test_statuses_outside_the_order_rank_last_and_are_reported():
    ...
    # Review is unlisted, so its transitions are excluded from rework entirely.
...
-- A transition is rework when it moves backwards through the configured order.
-- A status absent from --status-order has no position, so its transitions are
-- EXCLUDED rather than positioned: you cannot tell whether an unknown status
-- sits before or after a known one, and guessing either way silently distorts
-- the count. derive prints the excluded statuses, which is the user's only
-- signal that the order is incomplete.
-- ponytail: exclusion, not inference. Upgrade path if it matters: infer a
-- position from observed transition frequency between known statuses.
SELECT t.key, t.ts, t.from_status, t.to_status, t.author_id
FROM transitions t
JOIN status_order sf ON sf.status = t.from_status
JOIN status_order st ON st.status = t.to_status
WHERE st.pos < sf.pos;
"""
...
def derive(con, status_order, start_status, review_status):
    ...
    if unknown:
        print("statuses not in --status-order, so their transitions are excluded "
              "from rework: "
...
def test_rework_is_attributed_to_the_sprint_it_happened_in():
    ...
    assert con.execute(chart.sql).fetchall() == [("Sprint B", 1)]
...
def test_carry_over_counts_tickets_not_memberships():
    ...
    # Sprint A is the later of the two sprints PROJ-3 belongs to; the fixtures
    # name sprints by identity (id 7 "Sprint B" then id 3 "Sprint A") so that
    # array order disagrees with both id and name order.
    assert rows["Sprint A"] == 1
