...
def client(reg):
    """A Flask test client. webapp is imported here rather than at module scope
    because the fixtures are written before webapp.py exists."""
    import webapp
...
`projects` does not exist until Task 1 lands, and Tasks 1 and 4 run concurrently.
Import it at module scope anyway: `test_helpers.py` is only ever imported by a test
file, and by the time any of them runs, the wave has been merged. If Task 1 has not
landed when this task's own tests run, `configured_db` still works, so run this
task's tests and note the import in the report rather than working around it.
...
Expected: the first test fails because `/alpha/` 404s without the blueprint. Confirm each of the seven individually.
