...
| `test_projects.py`, `test_wizard.py`, `test_webapp.py`, `test_container.py` | New tests, one file per concern. |
...
Wave 1  T1 projects.py        T2 wizard.py        T3 container      T4 urd/render helpers
        ...
        tests/run.sh                                                Dockerfile/compose
                                    |
...
Maximum concurrency is four. Two notes for whoever dispatches:

- **T4 is the only Wave 1 task touching existing files** (`urd.py`, `render.py`). It owns `test_webapp.py` so that Wave 2's T6 and Wave 3's T7 to T9 have somewhere to append without racing T1's or T2's test files.
- **Wave 3's three tasks are parallel only because the routes live in three blueprints.** If an implementer collapses them into `webapp.py`, the wave becomes serial. Keep the split.
...
set -eu
cd "$(dirname "$0")/.."
for f in test_urd.py test_projects.py test_wizard.py test_webapp.py test_container.py; do
	[ -f "$f" ] || continue
	printf '== %s\n' "$f"
...
- Produces: `urd.report_html(con, tiers=None) -> str`, `render.notice(title, lines, actions=()) -> str` where `actions` is a sequence of `(label, href, method)` triples.
...
def _configured_db():
    con = urd.open_db(os.path.join(tempfile.mkdtemp(), "t.duckdb"))
    urd.save_scope(con, site="example.atlassian.net", email="a@b.c", project="PROJ",
                   component="TEAM", earliest_since="2026-01-01",
                   status_order="To Do,In Progress,Review,Done",
                   start_status="In Progress", review_status="Review")
    return con
...
def test_report_html_writes_no_file():
    con = _configured_db()
    urd.derive(con, "To Do,In Progress,Review,Done", "In Progress", "Review")
    before = set(os.listdir(os.path.dirname(con.execute("SELECT 1").fetchone() and
                                            tempfile.gettempdir())))
    urd.report_html(con)
...
- [ ] **Step 4: Run every test file and the linter**
...
```bash
git add urd.py render.py test_webapp.py
...
def _registry(tmp=None):
    return projects_mod.ProjectRegistry(tmp or tempfile.mkdtemp())
...
def create_app(registry):
    ...
    # Imported here rather than at module scope: each blueprint arrives in its own
    # task, and a missing one must not stop the app from starting.
    for module in ("views_report", "views_jobs", "views_wizard"):
        try:
            app.register_blueprint(__import__(module).bp)
        except ImportError:
            pass
...
**Interfaces:**
- Consumes: `webapp.slug_or_404`, `webapp.project_page`, `urd.parse_thresholds`, `urd.stored_thresholds`, `urd.set_report_window`, `urd.set_excluded_epics`, `urd.set_min_closed`, `urd.stored_*`.
- Produces: `views_report.bp`, a `flask.Blueprint`. Routes `/` and `/<slug>/`. `views_report.flags_from(request, project) -> dict` with keys `since`, `epics`, `min_closed`, `tiers`.

- [ ] **Step 1: Write the failing tests**

```python
# append to test_webapp.py
def _synced(registry, slug="alpha"):
    project = registry.add(slug)
    urd.save_scope(project.con, site="example.atlassian.net", email="a@b.c",
                   project="PROJ", component="TEAM", earliest_since="2026-01-01",
                   status_order="To Do,In Progress,Review,Done",
                   start_status="In Progress", review_status="Review",
...
Run: `uv run --with duckdb --with flask python test_webapp.py`
Expected: the first new test fails because `/alpha/` 404s without the blueprint. Confirm each of the six individually.
...
```bash
git add views_report.py test_webapp.py
...
**Interfaces:**
- Consumes: `webapp.slug_or_404`, `projects.start_refresh`.
...
def refresh(slug):
    ...
    if not projects.start_refresh(project):
        # Refused rather than queued, so say so on the page it returns to.
        flask.flash("A refresh is already running for this project.")
...
`flask.flash` needs a secret key and a template to render into, neither of which this app has. Use a simpler mechanism instead: pass the refusal through the redirect as a query parameter and have `views_report` surface it.

```python
# views_jobs.py, replacing the refresh body
@bp.post("/<slug>/refresh")
def refresh(slug):
    registry = flask.current_app.config["REGISTRY"]
    project = webapp.slug_or_404(registry, slug)
...
And in `views_report.flags_from`, add to `problems` when the parameter is present:
...
```bash
git add views_jobs.py views_report.py test_webapp.py
...
**Interfaces:**
- Consumes: `wizard.Proposal`, `wizard.validate`, `wizard.apply`, `projects.ProjectRegistry.add`, `urd.token`, `render.notice`.
...
```bash
git add views_wizard.py test_webapp.py
...
**Placeholder scan.** No TBD, no "add error handling", no "similar to Task N". Task 7 and Task 8 each contain a correction to their own first code block (the `exclude_epic` splitting, and `flask.flash` being replaced by a query parameter) rather than a forward reference, because an implementer reading only that task needs the final form.

**Type consistency.** `JobState.as_dict` is defined in Task 1 and consumed in Task 8. `Project.configured()` is defined in Task 1 and used in Tasks 5, 6, 7, 10. `wizard.Result` fields `ok`, `who`, `issues`, `problem` are defined in Task 2 and read in Task 9. `flags_from` returns `since`, `epics`, `min_closed`, `tiers`, `problems`; Task 7's `_controls` reads all five and Task 8 appends to `problems`. `report_html(con, tiers=None)` is defined in Task 4 and called in Task 6.
